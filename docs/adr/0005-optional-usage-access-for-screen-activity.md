...
The product idea includes reminders after the user has been active on the phone for a configurable period. The current V1 implementation uses pending tasks, review cooldown, snooze, quiet hours, notifications, and WorkManager scheduling. It does not yet detect real cross-app screen activity.
...
V1 remains notification-first and elapsed-review-interval based. It must not request Usage Access during first-run onboarding.

Real screen-activity detection may be added later only as an optional advanced mode using Usage Access. The user must be able to keep using the app without granting Usage Access.

The future Usage Access mode should:
...
- Gives future Codex sessions a clear technical path for optional real screen detection.

Tradeoffs:

- V1 cannot honestly claim exact "screen active for 10 minutes" behavior.
...
- The future implementation must include emulator and physical-device testing before beta.
